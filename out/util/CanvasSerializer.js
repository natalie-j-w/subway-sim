export class CanvasSerializer {
    ...
    async JSONtoCanvas(json) {
        ...
        config.stations.forEach(stObj => {
            this.canvasHandler.createStation({ x: stObj.x, y: stObj.y }, stObj.id, stObj.name);
        });
        config.tracks.forEach(trObj => {
            this.canvasHandler.createTrack({ "startpoint": this.getStationById(trObj.from_station_id),
